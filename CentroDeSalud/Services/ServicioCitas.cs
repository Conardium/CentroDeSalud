    ...
    public class ServicioCitas : IServicioCitas
    {
        ...
        public async Task<ResultadoOperacion<List<TimeSpan>>> ListarHorasDisponibles(Guid medicoId, DateTime fecha)
        {
            ...
            if (disponibilidadMedico is null)
                return ResultadoOperacion<List<TimeSpan>>.Error("El médico no trabaja para el día seleccionado. Recuerde que puede comprobar la disponibilidad de un médico accediendo al perfil público de este dentro de la web.");
